...
link = 'https://gist.github.com/SaoasBlubb/7dcffa4c2eece15dd15ba83446357f5e/raw/versions.txt'
...
def check_update():
    ...
    api_url = 'https://api.github.com/repos/SaoasBlubb/YoutubeViewbot/releases/latest'
    response = requests.get(api_url, timeout=30)

    RELEASE_VERSION = response.json()['tag_name']

    if SCRIPT_VERSION != RELEASE_VERSION:
        print(bcolors.BOLD + 'Update Verfuegbar!!! ' +
              f'YouTube Viewer version {SCRIPT_VERSION} braucht ein Update auf version {RELEASE_VERSION}.' + bcolors.ENDC)
        url='https://github.com/SaoasBlubb/YoutubeViewbot/releases/download/' + RELEASE_VERSION + '/YouTubeViewbot.zip'
...
def download_driver():
    ...
    print(bcolors.WARNING + 'Lade Chrome Driver herunter...' + bcolors.ENDC)
    ...
    if OSNAME == 'Linux':
        ...
        if not version:
            print(bcolors.WARNING +
                  "Ihre Google Chrome-Version konnte nicht automatisch gefunden werden!" + bcolors.ENDC)
            version = input(bcolors.WARNING +
                            'Bitte geben Sie Ihre Google Chrome Version ein (z.B.: 91.0.4472.114): ' + bcolors.ENDC)
    else:
        print('{} OS wird nicht unterstuetzt.'.format(OSNAME))
        sys.exit()
...
def load_url():
    print(bcolors.WARNING + 'Laden von Urls...' + bcolors.ENDC)

    with open('urls.txt', encoding="utf-8") as fh:
        links = [x.strip() for x in fh if x.strip() != '']

    print(bcolors.OKGREEN +
          f'{len(links)} url geladen von urls.txt' + bcolors.ENDC)

    return links


def load_search():
    print(bcolors.WARNING + 'Abfragen laden...' + bcolors.ENDC)
    ...
    print(bcolors.OKGREEN +
          f'{len(search)} Abfrage geladen von search.txt' + bcolors.ENDC)
...
def gather_proxy():
    proxies = []
    print(bcolors.OKGREEN + 'Sammle proxies ...' + bcolors.ENDC)
    ...
    for link in link_list:
        ...
        print(bcolors.OKGREEN +
              f'{len(proxy)} Proxies gesammelt von {link}' + bcolors.ENDC)
...
def main_viewer(proxy_type, proxy, position):
    try:
        ...
        if status == 200:
            try:
                print(timestamp() + bcolors.OKBLUE + f"Versucht {position} | " + bcolors.OKGREEN +
                      f"{proxy} | {proxy_type} --> Guter Proxy | Starten eines neuen Treibers..." + bcolors.ENDC)

                create_html({"#3b8eea": f"Versucht {position} | ",
                             "#23d18b": f"{proxy} | {proxy_type} --> Guter Proxy | Starten eines neuen Treibers..."})
                ...
                if 'consent' in driver.current_url:
                    print(timestamp() + bcolors.OKBLUE +
                          f"Versucht {position} | Umgehung der Einwilligung..." + bcolors.ENDC)

                    create_html(
                        {"#3b8eea": f"Versucht {position} | Umgehung der Einwilligung..."})
                ...
                if 'watching' in view_stat:
                    ...
                    print(timestamp() + bcolors.OKBLUE + f"Versucht {position} | " + bcolors.OKGREEN +
                          f"{proxy} --> {youtube} Gefunden : {output} | Dauer der Beobachtung : {duration} " + bcolors.ENDC)

                    create_html({"#3b8eea": f"Versucht {position} | ",
                                 "#23d18b": f"{proxy} --> {youtube} Gefunden : {output} | Dauer der Beobachtung : {duration} "})
                ...
                print(timestamp() + bcolors.OKCYAN +
                      f'View hinzugefuegt : {view_count}' + bcolors.ENDC)

                create_html({"#29b2d3": f'View hinzugefuegt : {view_count}'})
                ...
                print(timestamp() + bcolors.FAIL +
                      f"Versucht {position} | Langsame Internetgeschwindigkeit oder beim recaptcha haengen geblieben! YouTube kann nicht geladen werden..." + bcolors.ENDC)

                create_html(
                    {"#f14c4c": f"Versucht {position} | Langsame Internetgeschwindigkeit oder beim recaptcha haengen geblieben! YouTube kann nicht geladen werden..."})
                ...
                print(timestamp() + bcolors.FAIL +
                      f"Versucht {position} | Kann [{video_title}] nicht finden mit diesem Stichworten [{keyword}]" + bcolors.ENDC)

                create_html(
                    {"#f14c4c": f"Versucht {position} | Kann [{video_title}] nicht finden mit diesem Stichworten [{keyword}]"})
                ...
                *_, exc_tb = sys.exc_info()
                print(timestamp() + bcolors.FAIL +
                      f"Versucht {position} | Zeile : {exc_tb.tb_lineno} | " + str(e) + bcolors.ENDC)

                create_html(
                    {"#f14c4c": f"Versucht {position} | Zeile : {exc_tb.tb_lineno} | " + str(e)})
        ...
        print(timestamp() + bcolors.FAIL +
              f"Versucht {position} | Deine urls.txt Datei ist leer!" + bcolors.ENDC)

        create_html(
            {"#f14c4c": f"Versucht {position} | Deine urls.txt Datei ist leer!"})
        pass

    except SearchError:
        print(timestamp() + bcolors.FAIL +
              f"Versucht {position} | Deine search.txt Datei ist leer!" + bcolors.ENDC)

        create_html(
            {"#f14c4c": f"Versucht {position} | Deine search.txt Datei ist leer!"})
        pass

    except:
        print(timestamp() + bcolors.OKBLUE + f"Versucht {position} | " +
              bcolors.FAIL + f"{proxy} | {proxy_type} --> Schlechter Proxy " + bcolors.ENDC)

        create_html({"#3b8eea": f"Versucht {position} | ",
                     "#f14c4c": f"{proxy} | {proxy_type} --> Schlechter Proxy "})
...
def stop_server(immediate=False):
    ...
    if api and server_running:
        print('Versuch, den Server anzuhalten')
        if not immediate:
    ...
    with ThreadPoolExecutor(max_workers=threads) as executor:
        ...
        try:
            for future in as_completed(futures):
                ...
                if len(view) == views:
                    print(
                        bcolors.WARNING + f'Anzahl der hinzugefuegten Views : {views} | Programm beenden...' + bcolors.ENDC)
                    ...
                    if (time() - start_time) > refresh*60:
                        ...
                        print(bcolors.WARNING +
                              f'Proxy neu geladen von : {filename}' + bcolors.ENDC)

                        total_proxies = len(proxy_list)
                        print(bcolors.OKCYAN +
                              f'Proxies gesamt: {total_proxies}' + bcolors.ENDC)
...
if __name__ == '__main__':
    ...
    if os.path.isfile('config.json'):
        ...
        if len(config) == 11:
            ...
            previous = str(input(
                bcolors.OKBLUE + 'Die Konfigurationsdatei existiert! Moechten Sie mit den zuvor gespeicherten Einstellungen fortfahren? [Yes/No] : ' + bcolors.ENDC)).lower()
            if previous == 'n' or previous == 'no':
                create_config()
            else:
                pass
        else:
            print(bcolors.FAIL + 'Die vorherige Konfigurationsdatei ist nicht mit dem aktuellen Skript kompatibel! Erstellen Sie eine neue Datei...' + bcolors.ENDC)
            create_config()
    else:
        create_config()
    ...
    if auth_required and background:
        print(bcolors.FAIL +
              "Premium-Proxy benoetigt eine Erweiterung, um zu funktionieren. Chrome unterstuetzt keine Erweiterungen im Headless-Modus." + bcolors.ENDC)
        input(bcolors.WARNING +
              f"Entweder Proxy ohne Benutzernamen und Passwort verwenden oder Headless-Modus deaktivieren " + bcolors.ENDC)
        sys.exit()
    ...
    while len(view) < views:
        try:
            ...
            if check == 0:
                ...
                sleeping()
                print(bcolors.WARNING +
                      f'Insgesamt Geprueft : {check} mal' + bcolors.ENDC)
                main()
        except KeyboardInterrupt:
            sys.exit()
